...
def help(update, context):
    ...
    operazioniDesc = map(
        lambda x: '/{} , {} un libro.'.format(x, x), getOperations())
    text = '\n'.join(operazioniDesc)
...
def prenota(update, context):
    ...
    if len(msg) == 1:
        ...
        update.message.reply_text(
            "Per prenotare un libro scrivi /prenota seguito dal nome del libro che desideri")
    else:
        # ho anche il nome del libro
        # SECONDO ME BISOGNA METTERLO LOWER (msg[1].lower()) PER RENDERE PIU' FACILE LA RICERCA DEL LIBRO
        libro = msg[1]
        ...
        if r.text == '[]':
            update.message.reply_text(
                'Spiacenti, ma il libro {} non è disponibile al momento'.format(libro))
            return
